...
void SceneRaceAssignment2::Init() {
	...
	meshList[GEO_SKY_LEFT]->textureID = LoadTGA("Image//left.tga");
	meshList[GEO_SKY_RIGHT]->textureID = LoadTGA("Image//right.tga");
	meshList[GEO_SKY_TOP]->textureID = LoadTGA("Image//top.tga");
	meshList[GEO_SKY_BOTTOM]->textureID = LoadTGA("Image//bottom.tga");
	meshList[GEO_SKY_FRONT]->textureID = LoadTGA("Image//front.tga");
	meshList[GEO_SKY_BACK]->textureID = LoadTGA("Image//back.tga");
...
void SceneRaceAssignment2::Render()
{
	...
	*/
	...
	modelStack.PushMatrix();
	modelStack.Translate(20.0f, 0.0f, -5.0f);
	modelStack.Rotate(0, 0.0f, 1.0f, 0.0f);
	modelStack.Scale(1.0f, 1.0f, 1.0f);
	this->RenderMesh(meshList[GEO_MODEL1], lightEnable);
	modelStack.PopMatrix();
...
void SceneRaceAssignment2::RenderMesh(Mesh* mesh, bool enableLight)
{
...
void SceneRaceAssignment2::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y) {
...
void SceneRaceAssignment2::RenderText(Mesh* mesh, std::string text, Color color)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
