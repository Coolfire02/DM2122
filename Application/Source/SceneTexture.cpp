...
void SceneTexture::Init() {
	...
	mat.kShininess = 1.0f;
	meshList[GEO_QUAD] = MeshBuilder::GenerateQuad("cube", Color(0.9f, 0.6f, 0.3f));
	meshList[GEO_AXES] = MeshBuilder::GenerateAxes("axes", 1, 1, 1);
	meshList[GEO_SPHERE] = MeshBuilder::GenerateSphere("sphere", Color(1.0f, 1.0f, 1.0f));

	meshList[GEO_LIGHTBALL] = MeshBuilder::GenerateSphere("lightbulll", Color(1.0f, 1.0f, 1.0f));

	//meshList[GEO_QUAD]->material = mat;
...
void SceneTexture::Render()
{
	...
	*/
	...
	this->RenderMesh(meshList[GEO_QUAD], false);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(0.0f, 0.4f, 0.0f);
	modelStack.Rotate(00, 1.0f, 0.0f, 0.0f);
	modelStack.Scale(0.2f, 0.2f, 0.2f);
	this->RenderMesh(meshList[GEO_SPHERE], false);
	modelStack.PopMatrix();
