...
void SceneUI::Init() {
	...
	meshList[GEO_SONIC_AMY] = MeshBuilder::GenerateOBJMTL("amy", "OBJ//tree_large.obj", "MTL//tree_large.mtl");
...
void SceneUI::Render()
{
	...
	*/
	...
	this->RenderMesh(meshList[GEO_SONIC_AMY], lightEnable);
	modelStack.PopMatrix();
	...
	RenderMeshOnScreen(meshList[GEO_ICON], 40, 30, 20, 10);
...
void SceneUI::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y) {
	...
	for (unsigned i = 0; i < text.length(); ++i)
	{
		...
		characterSpacing.SetToTranslation(0.5f + i * 0.5f, 0.5f, 0); //i+x = width
